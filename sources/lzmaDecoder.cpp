                        ...
                        /* out */ unsigned int &nOutputBytesWritten )
{
	...
	size_t expectedInputRemaining = m_nCompressedSize - std::min( m_nCompressedBytesRead + nCompressedBytesRead, m_nCompressedSize );
	size_t expectedOutputRemaining = m_nActualSize - m_nActualBytesRead;
	size_t inSize = std::min( (size_t)nMaxInputBytes, expectedInputRemaining );
	size_t outSize = std::min( (size_t)nMaxOutputBytes, expectedOutputRemaining );
	ELzmaStatus status;
...
CLZMAStream::eHeaderParse CLZMAStream::TryParseHeader( unsigned char *pInput, unsigned int nBytesAvailable, /* out */ unsigned int &nBytesConsumed )
{
	...
	if ( m_bZIPStyleHeader )
	{
		...
		uint16_t nLZMAPropertiesSize = *(uint16_t *)(pInput + 2);
	...
	else
	{
		...
		m_nCompressedSize = ((lzma_header_t *)pInput)->lzmaSize + sizeof( lzma_header_t );
